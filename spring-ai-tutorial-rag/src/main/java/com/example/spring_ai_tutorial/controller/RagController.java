...
public class RagController {
    ...
    @Operation(summary = "인덱싱된 파일 목록 조회",
            description = "file/ 폴더의 하위 디렉토리 및 파일 목록을 반환합니다.")
    @ApiResponse(responseCode = "200", description = "파일 목록 조회 성공")
    @GetMapping("/files")
    public ResponseEntity<ApiResponseDto<List<FileListResponseDto>>> listFiles() {
        try {
            List<FileListResponseDto> files = ragService.listIndexedFiles();
            return ResponseEntity.ok(ApiResponseDto.success(files));
        } catch (Exception e) {
            log.error("파일 목록 조회 중 오류 발생", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponseDto.failure("파일 목록 조회 중 오류가 발생했습니다."));
        }
    }
